...
class SentinelWorkflow:
    ...
    def __init__(self, provider: DecisionProvider, policy: Policy | None = None) -> None:
        self.provider = provider
        self.policy = policy or Policy()

    def review(self, case: Case) -> Decision:
        state = {"case": redact(case.to_dict()), "policy": {"allowed_actions": sorted(self.policy.allowed_actions)}}
    ...
    def execute(
        ...
        *,
        ...
        authorization = self.policy.authorize(decision.action, user_approved=user_approved)
        result: dict[str, Any] = {
            "case": case.to_dict(),
        ...
        if not authorization["allowed"]:
            result["verification"] = Verification("not_executed", False, next_step="notify_user").to_dict()
            return result
        try:
            ...
            result["verification"] = Verification("dispatch_failed", False, next_step="notify_and_retry").to_dict()
            return result
        ...
        try:
            ...
            result["verification"] = {"verified": False, "status": "readback_failed", "error_type": type(exc).__name__, "next_step": "reopen_case"}
        return result
